...
StageScene::StageScene(GameManager* _gameManager, GameVersion _gameVersion, Players _Players, const unsigned int _level, const unsigned int _score)
    : Scene(_gameManager), gameVersion(_gameVersion), tipoPlayer(_Players), stage(_level), score(_score)
{
    // stage text
    auto text = std::make_shared<Text>(gameManager->getAssetManager()->getFont(), gameManager->getRenderer(),
        "ETAPA " + std::to_string(_level));
...
void StageScene::update(const unsigned int _delta)
{
    ...
    if(untilNextSceneTimer >= sceneTimer)
    {
       ...
       // gameManager->getSceneManager()->addScene("level", std::make_shared<LevelScene>(gameManager, gameVersion, stage, score));
        gameManager->getSceneManager()->addScene("level", std::make_shared<LevelScene>(gameManager, gameVersion, tipoPlayer, stage, score));
