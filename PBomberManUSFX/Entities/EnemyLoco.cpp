...
EnemyLoco::EnemyLoco(std::shared_ptr<SDL_Texture> _texture, SDL_Renderer* _renderer)
...
void EnemyLoco::moveTo(const int x, const int y)
{
...
void EnemyLoco::moveToCell(std::pair<int, int> pathToCell)
{
    // save moving path
    path = pathToCell;
    movingToCell = true;
...
bool EnemyLoco::isMovingToCell() const
{
    return movingToCell;
}

bool EnemyLoco::canAttack() const
{
    return aiType == AIType::chasing;
}

int EnemyLoco::getAttackRadius() const
{
    return attackRadius * getWidth();
}

void EnemyLoco::setAIType(AIType type)
{
    aiType = type;
}

void EnemyLoco::update(const unsigned int delta)
{
...
void EnemyLoco::updateMovement(const unsigned int delta)
{
...
void EnemyLoco::generateNewPath()
{
    // create random func
    ...
    if (randUpDownSign != 0)
    {
        moveTo(3, randUpDown * randUpDownSign * getHeight());
        return;
    }
